...
async def show_ps_page(update_or_query, context, session, page):
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("◀", callback_data=f"ps_prev_{page-1}"))
    nav.append(InlineKeyboardButton("🔄", callback_data="ps_refresh"))
    if page < total - 1:
        nav.append(InlineKeyboardButton("▶", callback_data=f"ps_next_{page+1}"))
    buttons.append(nav)
...
async def ps_callback(update: Update, context: CallbackContext):
    ...
    if data.startswith("ps_prev_") or data.startswith("ps_next_"):
